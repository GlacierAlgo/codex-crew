...
class StorageTests(unittest.TestCase):
    def test_upsert_is_idempotent_by_session_and_turn(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            first = StopSnapshot("session", "turn", 10, final_text="first", input_tokens=10)
            second = StopSnapshot("session", "turn", 11, final_text="second", input_tokens=12)
            upsert_snapshot(first, database)
            upsert_snapshot(second, database)

            with closing(sqlite3.connect(database)) as connection:
                count = connection.execute("SELECT COUNT(*) FROM turn_stop_snapshot").fetchone()[0]
    ...
    def test_summary_uses_only_latest_snapshot_per_session(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            database = Path(directory) / "snapshots.sqlite3"
            upsert_snapshot(
                StopSnapshot("one", "old", 10, input_tokens=10, cached_input_tokens=4,
                             output_tokens=2, reasoning_output_tokens=1, goal_tokens_used=9),
                database,
            )
            upsert_snapshot(
                StopSnapshot("one", "new", 20, input_tokens=20, cached_input_tokens=8,
                             output_tokens=5, reasoning_output_tokens=3, goal_tokens_used=15),
                database,
            )
            upsert_snapshot(
                StopSnapshot("two", "only", 15, input_tokens=7, cached_input_tokens=2,
                             output_tokens=3, reasoning_output_tokens=1, goal_tokens_used=4),
                database,
            )
