...
Worker 对同一个 system new-build 或 migration 请求独立给出设计，Commander 是唯一
user-facing communication thread，负责多轮派发、机械校验、比较、metrics 与用户下一步。

Worker 的唯一实验变量是各 role profile 固定的数值 budget `N`：恰好 `N` 个 deep
...
- schema v2 `manifest.toml` 是 ordered roles、exactly one `communication_role`、runtime
  profiles、model、reasoning effort、service tier 与显式 `split-plan` layout 的唯一 runtime
  authority。role order 固定为 `commander, worker_3, worker_4, worker_5, worker_6`，
  communication role 是 `commander`。
- `commander.md` 与四份 `worker_*.md` 是唯一 editable instruction sources；generated
  profile TOML 只是派生 adapter。
- 四个 Worker native `thread_id` 彼此独立，不读取其他 Worker 的上下文或输出。
  Commander 使用自己的 exact thread，接收用户 request 并控制四个 sub-threads。
- manifest split plan 按 role order 创建 panes：`worker_3` 从 `commander` horizontal split
  67%，`worker_4` 从 `worker_3` vertical split 50%，`worker_5` 从 `worker_3` horizontal
  split 50%，`worker_6` 从 `worker_4` horizontal split 50%。最终为三列近似等宽的
  column-major `[1,2,2]`：左列 `commander` full height，中列上/下是
  `worker_3`/`worker_4`，右列上/下是 `worker_5`/`worker_6`（左1/中23/右45）。
  dispatch、completion 与 final 都只使用 explicit Unix
  endpoint、exact native `thread_id` 和 exact `turn_id`。

## Shared experiment contract

- Commander 将用户原始 design request byte-identical 地只发送给四个 Worker exact
  threads；四次 request bytes 必须一致，request 中不要另行注入 budget 或 runtime handoff。
- 每个 Worker 必须同时产出恰好 `N` 个 counted deep modules 与恰好 `N` 个顶层
...
## Automatic handoff and Commander lifecycle

所有 identity bootstrap turns COMMIT 后，launcher 自动向 exact Commander thread 发送
...
role 与 ordered role -> exact pane/thread/bootstrap-turn mappings；不得转发给 Workers，
也不改变 native thread identity。Commander 保存 mappings 后，authoritative final 第一行
...
用户只与 Commander pane/thread 交互。收到一轮 design request 后，Commander：
...
2. 通过 existing exact `crew send`/`wait`/`final` 把 byte-identical request 分发给四个
   Workers；active turn 不重复 send，所有操作保留 exact `turn_id` precondition。
...
4. 再读取每个 thread 的 native goal；必须报告 goal `status`、`tokensUsed`、可选
   `tokenBudget`、`timeUsedSeconds`，以及 Commander-observed round wall elapsed。
...
同一 Commander thread 跨轮持续存在。用户选择结束并回收时，Commander 只在当前
